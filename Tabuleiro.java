...
public class Tabuleiro {
    ...
    public boolean marcarJogada(Jogador jog, Movimento mov) throws ExcecaoMovimentoInvalido{
        int i = mov.getI();
        int j = mov.getJ();
        
        
        if(i < 0 || j < 0 || i >= matriz.length || j >= matriz.length){
            throw new ExcecaoMovimentoInvalido("O intervalo da jogada é inválido");
        }
        
        if(matriz[i][j] != ' '){
            throw new ExcecaoMovimentoInvalido("Esta jogada já foi realizada!");
        }
